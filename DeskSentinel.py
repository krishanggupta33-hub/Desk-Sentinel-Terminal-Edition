...
class Game:
    ...
    def draw_hud(self):
        bar_w, bar_h = 260, 22
        bx, by = 20, 20
        pygame.draw.rect(self.screen, (30, 20, 20), (bx, by, bar_w, bar_h))
        pct = self.health / MAX_HEALTH
        fill_color = NEON_GREEN if pct > 0.5 else (NEON_CYAN if pct > 0.25 else NEON_RED)
        pygame.draw.rect(self.screen, fill_color, (bx, by, int(bar_w * pct), bar_h))
        pygame.draw.rect(self.screen, WHITE, (bx, by, bar_w, bar_h), width=2)
        label = self.font_ui.render(f"SERVER INTEGRITY {int(self.health)}%", True, WHITE)
        self.screen.blit(label, (bx, by + bar_h + 6))

        score_label = self.font_ui_big.render(f"{self.score:04d}", True, NEON_GREEN)
        self.screen.blit(score_label, (WIDTH - score_label.get_width() - 24, 16))
        score_caption = self.font_ui.render("PACKETS NEUTRALIZED", True, DIM_GREEN)
        self.screen.blit(
            score_caption, (WIDTH - score_caption.get_width() - 24, 16 + score_label.get_height())
        )

        level_label = self.font_ui.render(f"LEVEL {self.level}", True, NEON_CYAN)
        self.screen.blit(level_label, (WIDTH // 2 - level_label.get_width() // 2, 20))

        input_bg = pygame.Rect(WIDTH // 2 - 160, HEIGHT - 60, 320, 40)
        pygame.draw.rect(self.screen, (16, 20, 18), input_bg)
        border_color = NEON_GREEN if self.typed else DIM_GREEN
        pygame.draw.rect(self.screen, border_color, input_bg, width=2)
        typed_surf = self.font_block.render(self.typed + "_", True, NEON_GREEN)
        self.screen.blit(
            typed_surf,
            (input_bg.centerx - typed_surf.get_width() // 2, input_bg.centery - typed_surf.get_height() // 2),
        )
    ...
    def draw_game_over(self):
        ...
        title = self.font_title.render("SERVER BREACHED", True, NEON_RED)
        self.screen.blit(title, (WIDTH // 2 - title.get_width() // 2, HEIGHT // 2 - 110))

        stat = self.font_ui_big.render(
            f"Packets neutralized: {self.score}", True, WHITE
        )
        self.screen.blit(stat, (WIDTH // 2 - stat.get_width() // 2, HEIGHT // 2 - 30))

        stat2 = self.font_ui.render(
            f"Survived {self.time_alive:.1f}s   |   Reached Level {self.level}", True, NEON_CYAN
        )
        self.screen.blit(stat2, (WIDTH // 2 - stat2.get_width() // 2, HEIGHT // 2 + 20))

        hint = self.font_ui.render("Press R to restart   |   ESC to quit", True, DIM_GREEN)
        self.screen.blit(hint, (WIDTH // 2 - hint.get_width() // 2, HEIGHT // 2 + 70))

    def draw(self):
        self.screen.fill(BG_COLOR)
        self.draw_grid()

        pygame.draw.line(self.screen, (60, 15, 20), (0, HEIGHT - 70), (WIDTH, HEIGHT - 70), width=2)

        for b in self.blocks:
            b.draw(self.screen, self.font_block, is_target=(b is self.target_block))
